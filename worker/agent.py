...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'system','system_prompt'}: payload[name]='Evidence-grounded human factors analysis. CLAIM<=EVIDENCE.'
...
ROLE=os.getenv('ROLE','HUMAN_FACTORS_ANALYST'); MODEL=os.getenv('MODEL','huggingface-projects/llama-3.2-3B-Instruct'); mission=pathlib.Path('MISSION.md').read_text(encoding='utf-8')
prompt=f'''You are {ROLE} in CEREBRON Farm 30 Human Factors.\n{mission}\nProduce a concise evidence-disciplined assessment with task/context/population boundaries, mechanisms, failure modes, confounders, measurable variables, human-system interaction risks, field-vs-lab validity limits, mitigation options, validation path, unknowns, contradictions and a final claim ledger. Never infer intent from behavior without evidence. Never treat a simulation, model, benchmark or lab study as direct field validation.'''
